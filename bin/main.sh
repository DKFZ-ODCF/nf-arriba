...
# remove uninteresting viruses
samtools view -@ "$THREADS" -O "BAM,level=1" -e 'rname!~"'"$IGNORE_VIRUSES"'"' |
...
samtools sort -@ "$THREADS" -m "$((MEMORY/THREADS*85/100))M" -l 1 -T "$(pwd)" --write-index -o sorted.bam##idx##sorted.bam.bai alignments.bam
rm -f alignments.bam
